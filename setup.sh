...
if [ "$user" == "root" ]; then
	...
	if [ $? != 0 ] && [ $? != 130 ]; then
		echo -e "\n${redColour}[-] Failed to install bspwm using apt. Trying via build from repository.\n${endColour}"
	...
	if [ $? != 0 ] && [ $? != 130 ]; then
		echo -e "\n${redColour}[-] Failed to install sxhkd using apt. Trying build from repository.\n${endColour}"
		sleep 2
		...
		sudo make install
		if [ $? != 0 ] && [ $? != 130 ]; then
			echo -e "\n${redColour}[-] Failed to install sxhkd via build also...\n${endColour}"
			exit 1
		else
			echo -e "\n${greenColour}[+] Done\n${endColour}"
			sleep 1.5
	...
	if [ $? != 0 ] && [ $? != 130 ]; then
		echo -e "\n${redColour}[-] Failed to install polybar using apt. Trying build from repository.\n${endColour}"
		sleep 2
		...
		sudo make install
		if [ $? != 0 ] && [ $? != 130 ]; then
			echo -e "\n${redColour}[-] Failed to install polybar via build also...\n${endColour}"
			exit 1
		else
			echo -e "\n${greenColour}[+] Done\n${endColour}"
			sleep 1.5
	...
	echo -e "\n${purpleColour}[*] Installing picom compositor...\n${endColour}"
	sleep 2
	sudo apt install picom
	if [ $? != 0 ] && [ $? != 130 ]; then
		echo -e "\n${redColour}[-] Failed to install picom using apt. Trying build from repository.\n${endColour}"
		sleep 2
		...
		if [ $? != 0 ] && [ $? != 130 ]; then
			echo -e "\n${redColour}[-] Failed to install picom via build also...\n${endColour}"
			exit 1
		else
			echo -e "\n${greenColour}[+] Done\n${endColour}"
			sleep 1.5
	...
	echo -e "\n${purpleColour}[*] Installing eww widgets for the system...\n${endColour}"
	sudo apt install eww || cargo install eww
	if [ $? != 0 ] && [ $? != 130 ]; then
		echo -e "\n${redColour}[-] Failed to install eww widgets for the system via cargo or apt. Trying via build.\n${endColour}"
		bash eww_installer.sh
		if [ $? != 0 ] && [ $? != 130 ]; then
			echo -e "\n${redColour}[-] Failed to install eww widgets for the system via build...\n${endColour}"
			exit 1
		else
			echo -e "\n${greenColour}[+] Done\n${endColour}"
		fi
